...
{    
    public class Comunicacion
    {
        ...
        public Error Desconectar(bool interno = false)
        {
            ...
            {
                return EmpalmeErrorExcepcion(ex, ErrComunicacion.CONEXIONex, "Ha ocurrido un problema al intentar cerrar la conexión.", 1);
            }
            ...
            try
            {
                ...
                else if (TransacManager.ProtoConfig.TIPO_CXN == EnumModoConexion.DIALUP)
                {
                    ...
                    try
                    {
                        ...
                        if (port1.DsrHolding == true)
                        {
                            ...
                            if (bytepr.Length > 2 && NivelLog <= lvlLogDebug)
                                LogBMTP.LogBuffer(byteToChar(bytepr), "Bytes de Radio ajenos al protocolo normal. Normalmente no viene nada.", bytepr.Length, lvlLogDebug);
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                            Array.Resize(ref bytepr, con);
                            LogBMTP.LogBuffer(byteToChar(bytepr), "Superados intentos. Esto se recibió: ", bytepr.Length, lvlLogDebug);
                            throw e;
                        }
                    }
                }
                ...
                else if (TransacManager.ProtoConfig.TIPO_CXN == EnumModoConexion.RADIO)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                            Array.Resize(ref bytepr, con);
                            LogBMTP.LogBuffer(byteToChar(bytepr), "Bytes de Radio: fallaron " + intentos.ToString() + " intentos. Esto se recibió: ", bytepr.Length, lvlLogDebug);
                            throw e;
                        }
                    }
                }
                ...
                //if (NivelLog <= lvlLogDebug)
                    LogBMTP.LogBuffer(byteToChar(aRecibir), "Enmascarado ( " + aRecibir.Length.ToString() + "b )", aRecibir.Length, lvlLogDebug);
                #endregion
                ...
                LogBMTP.LogBuffer(byteToChar(salidaUnpack), "Recibe " + tipoMen.Substring(0, 1) + " ( " + salidaUnpack.Length.ToString() + "b )", salidaUnpack.Length, lvlLogTransaccion);
        ...
        private Error EmpalmeErrorExcepcion(Exception ex, ErrComunicacion codigoError, string MensajeUsuario, uint estado = 99)
        {
            ...
            if ((sender != null && sender.Connected) || (esPorPuertoSerie && port1.IsOpen))
            {
                if (estado == 99)
                    errEx.Estado = 1;
                else if (estado != 99)
                ...
                errEx.Estado = 0;
            LogBMTP.LogMessage("Excepción: " + errEx.CodError + " " + errEx.Descripcion, lvlLogExcepciones, TimeStampLog);
            LogBMTP.LogMessage("Excepción: " + ex.ToString(), lvlLogDebug, TimeStampLog);
            return errEx;
        }
        #endregion
        #region Conexion
